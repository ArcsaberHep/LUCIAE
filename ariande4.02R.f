...
C $Id: armcdi.F,v 0.9 1992/01/31 16:14:59 lonnblad Exp $
      ...
      SUBROUTINE ARMCDItmp(ARRNDX,ARRNDY,ARVETO)
...
C $Id: arpars.F,v 0.10 1992/03/04 19:36:20 lonnblad Exp $
      ...
      SUBROUTINE ARPARStmp(NSTART,NEND)
...
C...Rotate (typically from z axis to direction theta,phi)
 ...
 100    CONTINUE
      ...
      IF(DBTOT2.GT.1.0D-20) THEN
        IF(DBTOT2.GE.1.0D0) THEN
        write(*,*) 'NI=',NI,'DBTOT2=',DBTOT2
        DBTOT2=0.99999999999999D0
C       CALL ARERRM('ARROBO',14,0)
        ENDIF
...
C...Suitable for q-qbar emission
     ...
     $         (EXP(-YMIN)-EXP(-YMAX))/YINT
        ...
        IF(MSTA(25).EQ.1)ARVET5=ARVET5*(-BC1-B1)/(1.0D0-B1+Y1)
      RETURN
...
C**** END OF AREXMA ****************************************************
      END
C   THE FOLLOWING SUBROUTINES ARE MODIFIED FROM 
C  ARIANDE FOR FIRECRACKER GLUON EMISSION
...
C...The Main driver routine in Ariadne.
      ...
      PARAMETER(MAXDIP=500,MAXPAR=500,MAXSTR=100,KSZ1=30,KSZ5=100)
...
C...Error if ARINIT has not been called
      IF(MSTA(2).EQ.0) CALL ARERRM('AREXEC',12,0)
        NS=1
c..calculate NS and construct a firecracker out of strings
        IF(KFR(15).EQ.1)CALL FRARSOR
        IF(NS.LE.1)THEN
        KFR15=KFR(15)
        KFR(15)=0
        ENDIF
...
c*****************************************************************
c..NR number of entry of string proceeding radiation
c..RPT2(100) Pt square of gluons from colour rope.
c..IFT(J)  =1 the gluon with Pt^2=RPT2(J) is accepted by J-th string
c..NPR number of gluons which has been emitted by rope = NS
C..NAC number of gluons which has been accepted by strings 
c..PTD2(J) pt^2 of firecracker gluon accepted by J-th string of the
c..cluster.=0.0 if the gluon is rejected
c**************************************************************
...
C...colour singlets of triplets are found
        ...
        IARSIG=1
c       CALL ARERRM('ARPARS',2,I)
        return
        endif
            IF(ABS(ITYP).EQ.0) then
c       CALL ARERRM('ARPARS',1,I)
        IARSIG=1
        return
        endif
...
C...........If ordinary string create EM-dipole
            ELSE
              IF(ITYP.NE.-IDIR) then
c       CALL ARERRM('ARPARS',5,I)
        IARSIG=1
        return
        endif
              IF(MSTA(20).GT.0.AND.IDIPS.EQ.1.AND.
...
C...Initialize string variables in dipole record and perform cascade
            ...
            IF(MSTA(14).GT.1.AND.IPART.GT.2) PT2LST=ARMIPT(1,IPART)
            IF(PARA(6).GT.0.0) PT2LST=MIN(PT2LST,PARA(6)**2)
C...Don't cascade purely gluonic string
            IF(IDIR.NE.2) THEN
              IPF(1)=1
              IPL(1)=IPART
              ISTRS=1
              IFLOW(1)=IDIR
              CALL AREXMA(1,IPART)
              IF(KFR(15).EQ.1.AND.IS.EQ.2.AND.KFR(16).EQ.1)THEN
                CALL FRARGLU

        ENDIF
              IF(KFR(15).EQ.1)NR=IS     
                
              CALL ARCASC
c..because radiation fron rope is not dumped. decay it by hand
        IF(NR.EQ.1)THEN
        K(1,1)=12
        K(2,1)=11
        ENDIF
...
C...Performs a colour dipole cascade on string put in the ariadne
C...event record.
C If kfr(15)=0,then atoumatically NS=1,NR=2,IFT(1)=0.Colour rope becomes
c useless.If NS=1,then kfr(15)=0 atoumatically
...
C...Calculate total momentum of strings for debugging
      IF(MSTA(9).GT.0) CALL ARCHEM(1)
        ...
        IO=0
C..C...TAI
C...Adust parameters for radiation of rope      
        IFTG=1
                    IF(NR.EQ.1)THEN
c..colour rope emits gluons
        MSTAT=MSTA(6)
        MSTA(6)=NS
C THINK OF HERE AGAIN !!
c..string picks a gluon from firecracker gluons
                    ELSEIF(KFR(2).EQ.0.AND.KFR(15).EQ.1)THEN
        IF(IFT(NR-1).EQ.1)THEN  
        MSTAT=MSTA(6)
        MSTA(6)=1
        ELSE
        MSTAT=MSTA(6)
        MSTA(6)=0
        ENDIF
                    ELSEIF(KFR(2).EQ.0.AND.NS.EQ.1)THEN
        MSTAT=MSTA(6)
        MSTA(6)=0
                    ELSE
            ENDIF
        ...
        IF(NR.EQ.1)THEN
        ELSEIF(IO.EQ.0.AND.IFT(NR-1).EQ.1)THEN
        PARA10T=PARA(10)
        PARA(10)=0
        ELSE
...
C...Loop over all dipole to find largest possible p_t^2
 ...
 110  CONTINUE
        ...
        IF(NR.EQ.1)THEN
        ELSE
        IF(IO.EQ.0.AND.(IFT(NR-1).EQ.1.OR.(IFT(NR-1).
     &EQ.0.AND.IFTG.EQ.0)))PARA(10)=PARA10T
        ENDIF
             
C..TAI
C...Check that largest p_t^2 is above cuts.
      IF(ISEL.GT.0.AND.NR.NE.1) THEN
        IF((QEM(ISEL).AND.PT2MAX.LE.PARA(5)**2).OR.
     $     ((.NOT.QEM(ISEL)).AND.PT2MAX.LE.PARA(3)**2)) ISEL=0
      ENDIF
        IF(ISEL.EQ.0.AND.NR.EQ.1)ISEL=1
      IF(MSTA(6).GE.0.AND.IOT.GE.MSTA(6)) ISEL=0
...
C...with the appropriate Monte Carlo subroutine
      ...
      IF(.NOT.QDONE(ID)) THEN
        ...
        IF(NR.EQ.1)THEN
          CALL ARGQCD(ID)
        ELSEIF(QEM(ID).AND.(IO.NE.0.OR.
     &  (IO.EQ.0.AND.IFT(NR-1).EQ.0))) THEN
          CALL ARGQED(ID)
        ELSE
          CALL ARGQCD(ID)
        ENDIF
        IF(NR.NE.1)QDONE(ID)=.TRUE.
...
C...Generates a p_t^2 for a possible QCD emission from dipole ID
      ...
      PARAMETER(MAXDIP=500,MAXPAR=500,MAXSTR=100,KSZ5=100)
...
C...Exit if gluon emission was chosen 
 300  IF(IRAD(ID).EQ.0) RETURN
        NQUARK(IRAD(ID))=NQUARK(IRAD(ID))+1
C...Select which gluon to split
      IF((.NOT.QG3).OR.(QG1.AND.RLU(0).GT.0.5)) THEN
        IRAD(ID)=-IRAD(ID)
        B1=BX1(ID)
        BX1(ID)=BX3(ID)
        BX3(ID)=B1
...
C...Generates x_1 and x_3 for a radiating dipole
      ...
      PARAMETER(MAXDIP=500,MAXPAR=500,MAXSTR=100,KSZ1=30)
...
C...Exit if below cut
        ISQ=0
c..if a gluon from rope has been rejected by a string 200 times,then simple
c..give up the gluon.
100     ISQ=ISQ+1
                IF(NR.EQ.1)THEN
                ELSE
         IF(ISQ.GT.200.AND.IFT(NR-1).EQ.1.AND.IO.EQ.0)THEN
        PARA(10)=PARA10T
C...Calculate maximum x_t^2 for extended dipole
      IF(NE1.GT.0.AND.NE3.EQ.0) XT2ME=((0.25*S*(PARA(10+NE1)**
     $                          PARA(10)))**(2.0/(2.0+PARA(10))))/S
...
C...Set maximum x_t^2
      ...
      IF(NE1.GT.0.OR.NE3.GT.0) XT2M=MIN(XT2M,XT2ME)
        ...
        IF(XT2M.LT.XT2C) GOTO 900
C...Generate random XT2
      XT2=ARRNDX()
C...C...TAI
        IF(NR.NE.1)THEN
        IF(ISQ.LE.200.AND.IO.EQ.0.AND.IFT(NR-1).EQ.1)then
        XT2=PTD2(NR-1)/S
        endif
        ENDIF
      IF(XT2.LT.XT2C) THEN
...
C...If below cuts set XT2 to 0
 900  B1=-BC1
      ...
      XT2=0.0
        IF(NR.NE.1)THEN
        IF(IO.EQ.0.AND.IFT(NR-1).EQ.1)THEN
        IFT(NR-1)=0
        IFTG=0
        NAC=NAC-1
        ENDIF
